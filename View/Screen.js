...
export default function Screen() {
  ...
  useEffect(() => {
    fetch("https://retoolapi.dev/mvysdC/data")
  ...
  const handleUpdate = (index, newNote) => {
    const oldNote = userState.note[index];

    dispatch(updateNote(oldNote, newNote));

    const userIndex = data.findIndex((user) => user.id === 1);
    const updatedData = [...data];
    updatedData[userIndex].job[index] = newNote;

    fetch(`https://retoolapi.dev/mvysdC/data/1`, {
      method: "PUT",
      headers: {
            ...
            <FlatList
              ...
              renderItem={({ item, index }) => (
                ...
                >
                  ...
                  <TouchableOpacity onPress={() => handleUpdate(index,note1)}>
                    <Text
                      style={{
                        marginRight: 10,
                        fontSize: 15,
